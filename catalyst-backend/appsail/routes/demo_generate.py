...
from services.ai.cloudflare_llm import (
...
demo_generate_bp = Blueprint("demo_generate", __name__)


# ── 1. Prompt + Manual Form Endpoint ──

@demo_generate_bp.route("", methods=["POST"])
def demo_generate():
    """
    ...
    response = _build_response(floor_data_list, building_type, prompt, pw, pl)
...
@demo_generate_bp.route("/upload", methods=["POST"])
def demo_upload_and_process():
    ...
    if ext not in allowed:
        return jsonify({"detail": f"Unsupported file type '{ext}'. Allowed: {list(allowed)}"}), 400
    ...
    if image is None:
        return jsonify({"detail": "Could not decode the uploaded file as an image"}), 400
    ...
    total_area = sum(r.get("area_sqm", 0) for f in floors for r in f.get("rooms", []))
...
def _run_cv_ai_pipeline(image, building_type, is_pdf=False):
    """
            ...
            for ai_room in ai_analysis["rooms"]:
                ...
                if 0 <= idx < len(det_rooms):
                    ...
                    if suggested_type and ai_confidence >= 0.5:
                        ...
                        det_rooms[idx]["confidence"] = round((original_conf + ai_confidence) / 2, 3)
