...
Kept as-is from the original backend — no Catalyst dependency.
"""
...
CF_API_BASE = f"https://api.cloudflare.com/client/v4/accounts/{settings.CF_ACCOUNT_ID}/ai/run/"
...
def _call_llama(messages: List[Dict[str, str]], max_tokens: int = 2048) -> Optional[str]:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(f"Cloudflare AI API returned {response.status_code}: {response.text[:200]}")
            return None
